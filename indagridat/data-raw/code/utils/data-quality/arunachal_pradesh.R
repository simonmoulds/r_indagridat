...
state_energ_src_frac <- state_energy_frac(x=yrs, y=micensus_energ, state="Arunachal Pradesh", glm=FALSE)
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Cultivated","Other_Fallow","Current_Fallow","Net_Cropped_Area","Tree_Crops"), na.ix=list(Current_Fallow=c(44:57), Other_Fallow=c(44:57), Net_Cropped_Area=c(44:57), Tree_Crops=c(44:57)), legend.pos="topleft")
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Grass","Permanent_Pasture","Culturable_Waste"), na.ix=list(Permanent_Pasture=c(44:57), Culturable_Waste=c(44:57)))
ludata <- get_lu_data(y=ludata)
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Non_Agricultural_Uses"), na.ix=list(Non_Agricultural_Uses=c(44:57)))

## barren <- ludata$Barren_and_Unculturable
## barren[52:57] <- barren[52:57] - (barren[52] - barren[51])
## ludata$Barren_and_Unculturable <- barren

ludata <- check_data(x=yrs1, y=ludata, columns=c("Barren_and_Unculturable"), na.ix=list(Barren_and_Unculturable=c(44:57)))
