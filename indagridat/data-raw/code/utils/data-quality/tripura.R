...
state_energ_src_frac <- state_energy_frac(x=yrs, y=micensus_energ, state="Tripura", glm=FALSE, na.ix=37)
fv_frac              <- state_fv_frac(x=yrs, y=agcensus_fv, state="Tripura", glm=FALSE)

## *** Uncomment this section for irrigation data ***

## ## 2. land use + irrigation data
## data <- agcensus[agcensus$State %in% "Tripura",]
...
## area <- sum(unique(indiastat1950$Area[indiastat1950$State %in% "Tripura"]))
## ylim <- c(0,area)
...
## gw_frac    <- district_groundwater_frac(x=yrs, y=data, useGIA=FALSE, glm=T, na.ix=36) 
...
forest[1:11] <- forest[1:11] - (forest[11] - forest[12])
forest[20:27] <- forest[20:27] + (forest[28] - forest[27])
forest[20:57] <- forest[20:57] + (forest[19] - forest[20])
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Forest"), na.ix=list(Forest=c(38:46,21:23)))

cwaste <- ludata$Culturable_Waste
cwaste[16:19] <- cwaste[16:19] - (cwaste[16] - cwaste[15])
...
ppastur[12:19] <- ppastur[12:19] + (ppastur[11] - ppastur[12])
ppastur[22:26] <- ppastur[22:26] - (ppastur[22] - ppastur[20])
ludata$Permanent_Pasture <- ppastur

ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Grass","Permanent_Pasture","Culturable_Waste"), na.ix=list(Permanent_Pasture=c(20:31), Culturable_Waste=c()))
ludata <- get_lu_data(y=ludata)
...
nonagri[20:57] <- nonagri[20:57] - (nonagri[20] - nonagri[19])
nonagri[51:57] <- nonagri[51:57] - (nonagri[51] - nonagri[50])
ludata$Non_Agricultural_Uses <- nonagri

ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Non_Agricultural_Uses"), na.ix=list(Non_Agricultural_Uses=c(38:46)))
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Barren_and_Unculturable"), na.ix=list(Barren_and_Unculturable=c(20:42)), legend.pos="topleft")
...
## state_energ_src_frac <- getStateEnergySourceFrac(x=yrs, y=mi_energ_src, state="Tripura", glm=TRUE, na.ix=c(37))

## ## fruit/veg frac from agricultural census
## fv_frac <- readRDS("data/modified/agcensus/agcensus_statewise_veg_frac.rds")
## fv_frac <- getFruitVegFrac(x=yrs, y=fv_frac, state="Tripura")
...
## data_agcensus <- data_agcensus[data_agcensus$State %in% "Tripura",]
...
## ix1 <- data_agcensus$State %in% "Tripura"
