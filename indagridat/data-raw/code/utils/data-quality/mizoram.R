...
state_energ_src_frac <- state_energy_frac(x=yrs, y=micensus_energ, state="India")
fv_frac              <- state_fv_frac(x=yrs, y=agcensus_fv, state="Mizoram", glm=FALSE)

## *** Uncomment this section for irrigated area data ***

## ## 2. land use + irrigation data
## data <- agcensus[agcensus$State %in% "Mizoram",]
...
## area <- sum(unique(indiastat1950$Area[indiastat1950$State %in% "Mizoram"]))
## ylim <- c(0,area)
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Cultivated","Other_Fallow","Current_Fallow","Net_Cropped_Area","Tree_Crops"), na.ix=list(Current_Fallow=c(54:57), Other_Fallow=c(54:57), Net_Cropped_Area=c(54:57), Tree_Crops=c(54:57)), legend.pos="bottomright")
...
forest[19:34] <- forest[19:34] - (forest[19] - forest[17])
forest[1:34] <- forest[1:34] + (forest[49] - forest[34])
ludata$Forest <- forest

ludata <- check_data(x=yrs1, y=ludata, columns=c("Forest"), na.ix=list(Forest=c(18,35:48)))

cwaste <- ludata$Culturable_Waste
cwaste[43:57] <- cwaste[43:57] + (cwaste[34] - cwaste[43])
ludata$Culturable_Waste <- cwaste

ppastur <- ludata$Permanent_Pasture
ppastur[19:34] <- ppastur[19:34] + (ppastur[18] - ppastur[19])
ludata$Permanent_Pasture <- ppastur

ludata <- check_data(x=yrs1, y=ludata, columns=c("Grass","Permanent_Pasture","Culturable_Waste"), na.ix=list(Permanent_Pasture=c(35:57), Culturable_Waste=c(1:10,35:42,54:57)))
ludata <- get_lu_data(y=ludata)
...
## barren[1:17] <- barren[1:17] - (barren[17] - barren[19])
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Barren_and_Unculturable"), na.ix=list(Barren_and_Unculturable=c()), legend.pos="topleft")
...
## state_energ_src_frac <- getStateEnergySourceFrac(x=yrs, y=mi_energ_src, state="India", glm=TRUE)

## ## fruit/veg frac from agricultural census
## fv_frac <- readRDS("data/modified/agcensus/agcensus_statewise_veg_frac.rds")
## fv_frac <- getFruitVegFrac(x=yrs, y=fv_frac, state="Mizoram")
...
## data_agcensus <- data_agcensus[data_agcensus$State %in% "Mizoram",]
...
## ix1 <- data_agcensus$State %in% "Mizoram"
