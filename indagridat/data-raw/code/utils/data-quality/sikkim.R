...
state_energ_src_frac <- state_energy_frac(x=yrs, y=micensus_energ, state="India", glm=FALSE)
...
## data <- agcensus[agcensus$State %in% "Sikkim",]
...
## area <- sum(unique(indiastat1950$Area[indiastat1950$State %in% "Sikkim"]))
## ylim <- c(0,area)
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Forest"), na.ix=list(Forest=c(22,27:32)))
...
ppastur[27:57] <- ppastur[27:57] + (ppastur[26] - ppastur[27])
ludata$Permanent_Pasture <- ppastur

ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Grass","Permanent_Pasture","Culturable_Waste"), na.ix=list(Permanent_Pasture=c(), Culturable_Waste=c(24:26)))
ludata <- get_lu_data(y=ludata)
...
## fv_frac <- getFruitVegFrac(x=yrs, y=fv_frac, state="Sikkim")
...
## data_agcensus <- data_agcensus[data_agcensus$State %in% "Sikkim",]
...
## ix1 <- data_agcensus$State %in% "Sikkim"
