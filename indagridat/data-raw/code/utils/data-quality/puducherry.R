...
state_energ_src_frac <- state_energy_frac(x=yrs, y=micensus_energ, state="Puducherry", glm=FALSE)
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Forest"), na.ix=list(Forest=c(46,54:57)))
ludata$Forest <- 1
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Grass","Permanent_Pasture","Culturable_Waste"), na.ix=list(Permanent_Pasture=c(46), Culturable_Waste=c(31:36,46)))
ludata <- get_lu_data(y=ludata)

nonagri <- ludata$Non_Agricultural_Uses
nonagri[1:33] <-  nonagri[1:33] + (nonagri[34] - nonagri[33])
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Cultivated","Other_Fallow","Current_Fallow","Net_Cropped_Area","Tree_Crops"), na.ix=list(Current_Fallow=c(), Other_Fallow=c(), Net_Cropped_Area=c(), Tree_Crops=c()))
...
cwaste <- ludata$Culturable_Waste
cwaste[13:21] <- cwaste[13:21] - (cwaste[13] - cwaste[12])
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Grass","Permanent_Pasture","Culturable_Waste"), na.ix=list(Permanent_Pasture=c(7:11,22:28,46), Culturable_Waste=c(16:17,31:47,46)))
ludata <- get_lu_data(y=ludata)

nonagri <- ludata$Non_Agricultural_Uses
##nonagri[22:26] <- nonagri[22:26] + (nonagri[21] - nonagri[22])
nonagri[1:21] <- nonagri[1:21] - (nonagri[21] - nonagri[22])
nonagri[27:32] <- nonagri[27:32] - (nonagri[27] - nonagri[26])
nonagri[35:37] <- nonagri[35:37] - (nonagri[35] - nonagri[34])
nonagri[53:57] <- nonagri[53:57] - (nonagri[53] - nonagri[52])
nonagri[50:55] <- nonagri[50:55] - (nonagri[55] - nonagri[56])
nonagri[1:6] <-  nonagri[1:6] + (nonagri[7] - nonagri[6])
ludata$Non_Agricultural_Uses <- nonagri

ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Non_Agricultural_Uses"), na.ix=list(Non_Agricultural_Uses=c(1:15,38:42,46:48)))

barren <- ludata$Barren_and_Unculturable
barren[1:6] <- barren[1:6] - (barren[6] - barren[7])
ludata$Barren_and_Unculturable <- barren

ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Barren_and_Unculturable"), na.ix=list(Barren_and_Unculturable=c(1:24)))
...
## fraction of irrigation from groundwater
gw_frac    <- district_groundwater_frac(x=yrs, y=data, useGIA=TRUE, glm=FALSE, na.ix=c(25)) 
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Grass","Permanent_Pasture","Culturable_Waste"), na.ix=list(Permanent_Pasture=c(46), Culturable_Waste=c(31:57)))
ludata <- get_lu_data(y=ludata)
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Non_Agricultural_Uses"), na.ix=list(Non_Agricultural_Uses=c(1:45,53:57)))
...
## data <- d1
## data <- check_data(x=yrs, y=data, columns=names(data)[c(7:9,11:13,15:16,19)])
