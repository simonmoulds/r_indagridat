...
state_energ_src_frac <- state_energy_frac(x=yrs, y=micensus_energ, state="Nagaland", glm=FALSE, na.ix=c(37,57))
fv_frac              <- state_fv_frac(x=yrs, y=agcensus_fv, state="Nagaland", glm=FALSE)

## *** Uncomment this section for irrigation data ***

## ## 2. land use + irrigation data
## data <- agcensus[agcensus$State %in% "Nagaland",]
...
## area <- sum(unique(indiastat1950$Area[indiastat1950$State %in% "Nagaland"]))
## ylim <- c(0,area)
...
cwaste[1:25] <- cwaste[1:25] + (cwaste[26] - cwaste[25])
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Grass","Permanent_Pasture","Culturable_Waste"), na.ix=list(Permanent_Pasture=c(), Culturable_Waste=c(30:49,53,52:57)))
ludata <- get_lu_data(y=ludata)
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Barren_and_Unculturable"), na.ix=list(Barren_and_Unculturable=c(1:54)), legend.pos="topleft")
...
## state_energ_src_frac <- getStateEnergySourceFrac(x=yrs, y=mi_energ_src, state="Nagaland", glm=TRUE, na.ix=c(37,57))

## ## fruit/veg frac from agricultural census
## fv_frac <- readRDS("data/modified/agcensus/agcensus_statewise_veg_frac.rds")
## fv_frac <- getFruitVegFrac(x=yrs, y=fv_frac, state="Nagaland")
...
## data_agcensus <- data_agcensus[data_agcensus$State %in% "Nagaland",]
...
## ix1 <- data_agcensus$State %in% "Nagaland"
