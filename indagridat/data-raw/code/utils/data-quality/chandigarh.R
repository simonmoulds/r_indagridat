...
state_energ_src_frac <- state_energy_frac(x=yrs, y=micensus_energ, state="Chandigarh", glm=FALSE)
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Forest"), na.ix=list(Forest=c(27:57)))
## ludata$Forest <- 1
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Grass","Permanent_Pasture","Culturable_Waste"), na.ix=list(Permanent_Pasture=c(), Culturable_Waste=c(27:57)))
ludata <- get_lu_data(y=ludata)
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Non_Agricultural_Uses"), na.ix=list(Non_Agricultural_Uses=c(27:57)))
