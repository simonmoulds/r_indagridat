...
state_energ_src_frac <- state_energy_frac(x=yrs, y=micensus_energ, state="Goa", glm=FALSE)
...
cwaste[37:57] <- cwaste[37:57] + (cwaste[30] - cwaste[37])
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Non_Agricultural_Uses"), na.ix=list(Non_Agricultural_Uses=c(46,54:57)))
...
## irrigation
data$Rice_Winter[is.na(data$Rice_Winter)] <- data$Rice_Summer[is.na(data$Rice_Winter)]
