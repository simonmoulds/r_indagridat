...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Cultivated","Other_Fallow","Current_Fallow","Net_Cropped_Area","Tree_Crops"), na.ix=list(Current_Fallow=c(53:57), Other_Fallow=c(53:57), Net_Cropped_Area=c(53:57), Tree_Crops=c(53:57)), legend.pos="bottomright")

## forest <- ludata$Forest
## ## forest[1:11] <- forest[1:11] - (forest[11] - forest[12])
## ludata$Forest <- forest

ludata <- check_data(x=yrs1, y=ludata, columns=c("Forest"), na.ix=list(Forest=c()))
ludata$Forest <- 1
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Grass","Permanent_Pasture","Culturable_Waste"), na.ix=list(Permanent_Pasture=c(), Culturable_Waste=c(24:26,53:57)))
ludata <- get_lu_data(y=ludata)
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Non_Agricultural_Uses"), na.ix=list(Non_Agricultural_Uses=c(53:57)))
...
gw_frac <- data.frame(Year=yrs, SW_Frac=0.5, GW_Frac=0.5)
