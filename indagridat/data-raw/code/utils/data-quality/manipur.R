...
state_energ_src_frac <- state_energy_frac(x=yrs, y=micensus_energ, state="Manipur", glm=FALSE, na.ix=37)
fv_frac              <- state_fv_frac(x=yrs, y=agcensus_fv, state="Manipur", glm=FALSE)

## *** Uncomment this section for irrigated area data ***

## ## 2. land use + irrigation data
## data <- agcensus[agcensus$State %in% "Manipur",]
...
## ## ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Cultivated","Other_Fallow","Current_Fallow","Net_Cropped_Area","Tree_Crops"), na.ix=list(Current_Fallow=c(), Other_Fallow=c(), Net_Cropped_Area=c(1), Tree_Crops=c()), legend.pos="bottomright")
## ## ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Forest"), na.ix=list(Forest=c()))
## ## ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Grass","Permanent_Pasture","Culturable_Waste"), na.ix=list(Permanent_Pasture=c(32:37,42:44), Culturable_Waste=c()))
...
## ## ## ludata$Other_Fallow <- ludata$Other_Fallow - diff
...
## area <- sum(unique(indiastat1950$Area[indiastat1950$State %in% "Manipur"]))
## ylim <- c(0,area)
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Forest"), na.ix=list(Forest=c(1:4,12:14)))
...
ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Non_Agricultural_Uses"), na.ix=list(Non_Agricultural_Uses=c(1:4,35:57)))
...
barren[1:16] <- barren[1:16] + (barren[17] - barren[16])
ludata$Barren_and_Unculturable <- barren

ludata <- check_data(x=yrs1, y=ludata, columns=c("Area","Barren_and_Unculturable"), na.ix=list(Barren_and_Unculturable=c(12:14,35:57)), legend.pos="topleft")
...
## state_energ_src_frac <- getStateEnergySourceFrac(x=yrs, y=mi_energ_src, state="Manipur", glm=TRUE, na.ix=37)

## ## fruit/veg frac from agricultural census
## fv_frac <- readRDS("data/modified/agcensus/agcensus_statewise_veg_frac.rds")
## fv_frac <- getFruitVegFrac(x=yrs, y=fv_frac, state="Manipur")
...
## data_agcensus <- data_agcensus[data_agcensus$State %in% "Manipur",]
...
## ix1 <- data_agcensus$State %in% "Manipur"
